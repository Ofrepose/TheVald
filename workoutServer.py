...
app = Flask(__name__)
app.secret_key = '21234123413419adsfa'
...
from create_db_wo import Base, User, W, E, UserMixin, Stats, Common_E
...
def showLogin():
	...
	# featureThis = session.query(W).filter_by(name = 'Egypt').one()
	# featureThis.featured = True
	# session.add(featureThis)
	# session.commit();
	...
	all_users = session.query(User).all()
		...
		# print(a.p)
	# defaultUser = User(name_first = "dan", name_last = "pa", email = "paynedanielfranklin@gmail.com", p = "bright661")
...
def community():	
	...
	imageTester = ['za1.jpg','za2.jpg','za3.jpg','za4.jpg','za5.jpg','za6.jpg','za7.jpg','za8.jpg','za9.jpg','za10.jpg','za11.jpg','za12.jpg','za13.jpg'
	,'za14.jpg','za15.jpg','za16.jpg','za17.jpg','za18.jpg','za19.jpg']
...
# @app.route('/workout/upload/<int:workout_id>', methods = ['POST'. 'GET'])
# @login_required
# def uploadWorkout(workout_id):
# 	this_user = get_cur_user
# 	this_workout = session.query(W).filter_by(id = workout_id).one()
# 	if this_workout.original_owner == this_user.id:
...
def addExercise(thisWorkout_id):
	...
	if request.method == "POST":
		# for a in all_e:
		# 	if a.name.lower() == request.form['exerciseName'].lower():
...
def beginWorkout_weight(workout_id, exercise_id):
	...
	if request.method == 'POST':
		...
		# thisExercise.previous_weight = request.form['weight']
		session.add(thisExercise)
		session.commit()
...
def user_stats_name(name):
	...
	for i in all_stats :		
		if i.name_of == name:		
			...
			# print (i.name_of + ' was done on ' + str(i.date_done.date()))
	# for l in labels:
		# print ('labels is ' + l)
